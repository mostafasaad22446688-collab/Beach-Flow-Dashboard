...
function toInputTime(timeValue: string) {
  if (!timeValue) {
    return '';
  }
  ...
  const [clockPart, periodPart] = timeValue.split(' ');
  const [hoursText, minutesText] = (clockPart ?? '').split(':');
...
export function EditBeach() {
  const navigate = useNavigate();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const [beachId, setBeachId] = useState<number | null>(null);
  const [imagePreview, setImagePreview] = useState<string>('https://images.unsplash.com/photo-1643651564932-933066cbb895?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxiZWF1dGlmdWwlMjBiZWFjaCUyMHJlc29ydHxlbnwxfHx8fDE3NzI3NjQ1MjJ8MA&ixlib=rb-4.1.0&q=80&w=1080');

  // تحميل البيانات الحالية للشاطئ
  const [formData, setFormData] = useState({
    name: 'شاطئ الأحلام',
    location: 'الساحل الشمالي، مصر',
    description: 'شاطئ خاص فاخر يوفر تجربة استرخاء فريدة مع مياه صافية ورمال ناعمة. نقدم خدمات متميزة تشمل مطاعم راقية وأنشطة مائية متنوعة.',
    openTime: '08:00',
    closeTime: '20:00',
    pricePerDay: '50',
    capacity: '100',
  ...
  useEffect(() => {
    const loadBeach = async () => {
      try {
        const beaches = await adminApi.getMyBeaches<Array<Record<string, unknown>>>();
        const firstBeach = beaches?.[0];

        if (!firstBeach) {
          toast.error('لا يوجد شاطئ متاح للتعديل');
          navigate('/admin');
          return;
        }

        const idValue = Number(firstBeach.id);
        ...
        setFormData((prev) => ({
          ...prev,
          name: String(firstBeach.name ?? ''),
          location: String(firstBeach.location ?? ''),
          description: String(firstBeach.description ?? ''),
          openTime: toInputTime(String(firstBeach.openingTime ?? '')),
          closeTime: toInputTime(String(firstBeach.closingTime ?? '')),
          pricePerDay: String(firstBeach.price ?? ''),
          capacity: String(firstBeach.maxCapacity ?? ''),
        }));

        const preview = String(firstBeach.imageUrl ?? '');
        if (preview) {
          setImagePreview(preview);
        }
      } catch (error) {
        const description = error instanceof ApiError ? error.message : 'تعذر تحميل بيانات الشاطئ';
        toast.error('خطأ في التحميل', { description });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!beachId) {
      toast.error('تعذر تحديد الشاطئ المراد تحديثه');
      return;
    }
    ...
    try {
      await adminApi.updateBeach(beachId, {
        name: formData.name,
      ...
      toast.success('تم تحديث بيانات الشاطئ بنجاح! 🎉', {
      ...
      const description = error instanceof ApiError ? error.message : 'حدث خطأ أثناء تحديث بيانات الشاطئ';
      toast.error('تعذر حفظ التعديلات', { description });
    } finally {
    ...
    return (
      <div className="flex-1 bg-gradient-to-br from-cyan-50 via-blue-50 to-sky-50">
        <main className="px-6 lg:px-8 py-8 max-w-5xl mx-auto">
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-cyan-100 text-center text-gray-600">
            جاري تحميل بيانات الشاطئ...
          </div>
        </main>
      </div>
    );
          ...
          <div className="bg-gradient-to-r from-cyan-500 to-blue-600 rounded-2xl p-8 text-white shadow-xl mb-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h1 className="text-3xl font-bold mb-2">تعديل بيانات الشاطئ</h1>
                <p className="text-cyan-50 text-lg">
                  قم بتحديث معلومات الشاطئ. جميع الحقول المميزة بـ <span className="text-red-300">*</span> مطلوبة.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-cyan-100">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-2 flex items-center gap-2">
                  <AlertCircle className="w-4 h-4" />
                  اختر صورة جديدة لتحديث صورة الشاطئ (اختياري)
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-cyan-500 to-blue-600 rounded-2xl shadow-xl p-8 text-white">
            ...
            <div className="bg-white/10 rounded-xl p-6 backdrop-blur-sm mb-6">
              <p className="text-cyan-50 mb-4">
                تأكد من صحة التعديلات قبل الحفظ. سيتم تحديث بيانات الشاطئ مباشرة.
              </p>
